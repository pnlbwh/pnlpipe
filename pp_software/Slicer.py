import sys
from pp_software import getSoftDir, checkExists, TemporaryDirectory, prefixPATH, envFromDict
import logging
...
def make(version=DEFAULT_VERSION):
    if not URL.get(version):
        logging.error("No download link set for '{}' in pp_software/Slicer.py")
        sys.exit(1)
