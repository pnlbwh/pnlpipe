...
class DwiMaskSlicer(GeneratedNode):
    ...
    def build(self):
        ...
        with TemporaryDirectory() as tmpdir, local.cwd(
                tmpdir), pp_software.mrtrix3.env(self.hash_mrtrix3):
            from plumbum.cmd import maskfilter
            slicerDir = pp_software.Slicer.getPath(self.version_Slicer).dirname
...
class DiceCoefficient(GeneratedNode):
    ...
    def build(self):
        ...
        needDeps(self)
        with TemporaryDirectory() as tmpdir, pp_software.BRAINSTools.env(self.hash_BRAINSTools):
...
def status(combos):
    ...
    pp_pipelines = [makePipeline(**paramPoint) for paramPoint in pps]
    coeff_paths = [p['dice'].path() for p in pp_pipelines
