...
import pp_cli
from pp_cli import SRCPATHS, readAndSetSrcPaths, printVertical
from pp_cli.params import readParamCombos, readComboPaths, getSoftwareItems
import importlib
import logging
import pp_software
...
class Make(cli.Application):
    """ Builds necessary pp_software for pipeline and creates shell environment files. """
    ...
    def main(self):
        ...
        logging.info("Build prerequisite pp_software")
        ...
        with open(envFile, 'w') as f:
            ...
            for key, subjectPaths in comboPaths['paths'].items():
                ...
                if useFullPaths:
                    ...
                    from pp_cli.subcmd.symlink import toSymlink
            ...
            for softname, version, _ in getSoftwareItems(comboPaths[
                    'paramCombo']):
                m = importlib.import_module('pp_software.' + softname)
                if hasattr(m, 'envDict'):
                    envDicts.append(m.envDict(version))
            softVars = pp_software.composeEnvDicts(envDicts)
            for var, val in softVars.items():
                f.write('export {}={}\n\n'.format(var, val))
            f.write("export PATH={}:$PATH\n".format(
                local.path('pp_pipelines/pnlscripts')))
