...
from pp_cli import readAndSetSrcPaths, SRCPATHS
...
import pp_pipelines as pp_pipelinesDir
...
class Export(cli.Application):
    ...
    def main(self, *pipelineNames):
        ...
        if not pipelineNames:
            print("List the pp_pipelines for which you want a {} file generated.".format(PROJECT_YML))
            print
            self.help()
            sys.exit(1)
        ...
        for subprojectyml in (local.cwd // 'pnlproj-*yml'):
            with open(subprojectyml, 'r') as f:
                y = yaml.load(f)
            print("Found '{}', adding it to pp_pipelines in '{}'".format(subprojectyml, PROJECT_YML))
            pp_pipelines.append(y)
        ...
        for pipelineName in pipelineNames:
            ...
            pipelineModule = pp_pipelinesDir.importModule(pipelineName)
            ...
            for combo in combos:
                ...
                pp_pipelines.append(pipeline)

        result['pp_pipelines'] = pp_pipelines
